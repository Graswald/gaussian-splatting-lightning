...
def calculate_gaussian_scores(cameras, gaussian_model, device):
    ...
    opacity_score_list = []
    alpha_score_list = []
    all_visibility_score = torch.zeros((len(cameras), gaussian_model.get_xyz.shape[0]), dtype=torch.float,
                                       device=device)
    for idx, camera in tqdm(enumerate(cameras), total=len(cameras), leave=False,
                            desc="Calculating gaussian visibilities"):
...
def prune_and_get_weights(gaussian_model, cameras, n_average_cameras, weight_device: torch.device):
    ...
    visibility_score_acc_is_close_to_zero = torch.isclose(visibility_score_acc,
                                                          torch.tensor(0., device=visibility_score_acc.device))
    ...
    assert torch.all(torch.isclose(visibility_score_pruned_top_k_pdf.sum(dim=-1),
                                   torch.tensor(1., device=visibility_score_pruned_top_k_pdf.device)))
...
def average_color_fusing(
    ...
    rgb_offset_for_each_camera = torch.ones((gaussian_model.n_gaussians, n_average_cameras, 3), dtype=torch.float,
                                            device=device) * -1024.
    for i in range(n_chunk):
        ...
        camera_index_to_appearance_id = torch.tensor([i.appearance_id for i in cameras], dtype=torch.int,
                                                     device=visibility_score_pruned_sorted_indices.device)
        appearance_ids = camera_index_to_appearance_id[
            visibility_score_pruned_sorted_indices[..., left:right].reshape(-1)]  # [N_gaussians * real_chunk_size]
        ...
        if renderer.model_config.is_view_dependent:
            ...
            camera_centers = camera_index_to_camera_center[
                visibility_score_pruned_sorted_indices[..., left:right]]  # [N_gaussians, real_chunk_size, 3]
            view_directions = torch.nn.functional.normalize(gaussian_model.get_means().unsqueeze(1) - camera_centers,
                                                            dim=-1)  # [N_gaussians, n_average_cameras, 3]
        ...
        input_features = torch.concat(appearance_model_input_feature_list, dim=-1).reshape(
            (gaussian_model.n_gaussians * real_chunk_size, -1))

        rgb_offset_for_each_camera[:, left:right, :] = renderer.model.network(input_features).reshape(
            (gaussian_model.n_gaussians, real_chunk_size, -1)).to(device=device)
    ...
    weighted_rgb_offset_for_each_camera = rgb_offset_for_each_camera * visibility_score_pruned_top_k_pdf.unsqueeze(
        -1).to(device=device)
...
def average_embedding_fusing(
    ...
    camera_index_to_appearance_id = torch.tensor([i.appearance_id for i in cameras], dtype=torch.int,
                                                 device=visibility_score_pruned_sorted_indices.device)
    appearance_ids = camera_index_to_appearance_id[
        visibility_score_pruned_sorted_indices.reshape(-1)]  # [N_gaussians * n_average_cameras]
    # pick appearance embeddings
    appearance_embeddings = renderer.model.embedding(appearance_ids).reshape((
        visibility_score_pruned_sorted_indices.shape[0],
        n_average_cameras,
    ...
    if renderer.model_config.is_view_dependent:
        ...
        camera_centers = camera_index_to_camera_center[
            visibility_score_pruned_sorted_indices]  # [N_gaussians, n_average_cameras, 3]
        ...
        if view_dir_average_mode == "camera_center":
            ...
            unweighted_view_directions = torch.nn.functional.normalize(
                gaussian_model.get_means().unsqueeze(1) - camera_centers, dim=-1)  # [N_gaussians, n_average_cameras, 3]
            weighted_view_directions = unweighted_view_directions * visibility_score_pruned_top_k_pdf.unsqueeze(-1)
            view_directions = torch.nn.functional.normalize(torch.sum(weighted_view_directions, dim=1),
                                                            dim=-1)  # [N_gaussians, 3]
